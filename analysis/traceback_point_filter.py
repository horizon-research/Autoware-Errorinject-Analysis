...
def main():
    ...
    parser.add_argument('--base', default='filtered_points_attack_sdc')
    ...
    for i in range(len(f)):
        ...
        mid_frame = int(18/51*len(xl))
        start_frame = mid_frame - 3
        end_frame = mid_frame + 3

        xpavg.append(sum(xp[start_frame:end_frame])/len(xp[start_frame:end_frame]))
        ypavg.append(sum(yp[start_frame:end_frame])/len(yp[start_frame:end_frame]))
        zpavg.append(sum(zp[start_frame:end_frame])/len(zp[start_frame:end_frame]))
        xoavg.append(sum(xo[start_frame:end_frame])/len(xo[start_frame:end_frame]))
        yoavg.append(sum(yo[start_frame:end_frame])/len(yo[start_frame:end_frame]))
        zoavg.append(sum(zo[start_frame:end_frame])/len(zo[start_frame:end_frame]))
        woavg.append(sum(wo[start_frame:end_frame])/len(wo[start_frame:end_frame]))
        ...
        os.system(cmd)
    #pdb.set_trace()
    np.savetxt('xpavg.txt',xpavg)
    np.savetxt('ypavg.txt',ypavg)
    np.savetxt('zpavg.txt',zpavg)
    np.savetxt('xoavg.txt',xoavg)
    np.savetxt('yoavg.txt',yoavg)
    np.savetxt('zoavg.txt',zoavg)
    np.savetxt('woavg.txt',woavg)
