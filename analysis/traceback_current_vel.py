...
def find_injection_range(filename):
    ...
    flag = 0
    ori_xl, ori_yl, ori_zl, ori_xa, ori_ya, ori_za = parse_es_twist('/localdisk/autoware_attack_sdc/baseline148/current_velocity.txt')
    if signal == 'xl':
...
def find_injection_raw(xl_es,yl_es,zl_es,xa_es,ya_es,za_es,filename):
    file_str = filename.split('_')
    signal = file_str[2][6:8]
    q = deque(maxlen=8)

    if signal == 'xl':
        for i in range(len(xl_es)):
            if len(q) < 8:
                q.append(xl_es[i])
            else:
                avg,std = cavg(q)
                if (abs(xl_es[i] - avg) > 3*std and i > len(xl_es)/5):
                    return i 
                else:
                    q.append(xl_es[i])      
    if signal == 'yl':
        for i in range(len(yl_es)):
            if len(q) < 8:
                q.append(yl_es[i])
            else:
                avg,std = cavg(q)
                if (abs(yl_es[i] - avg) > 3*std and i > len(xl_es)/5):
                    return i 
                else:
                    q.append(yl_es[i])    
    if signal == 'zl':
        for i in range(len(zl_es)):
            if len(q) < 8:
                q.append(zl_es[i])
            else:
                avg,std = cavg(q)
                if (abs(zl_es[i] - avg) > 3*std and i > len(xl_es)/5):
                    #pdb.set_trace()
                    return i 
                else:
                    q.append(zl_es[i])    
    if signal == 'xa':
        for i in range(len(xa_es)):
            if len(q) < 8:
                q.append(xa_es[i])
            else:
                avg,std = cavg(q)
                if (abs(xa_es[i] - avg) > 3*std and i > len(xl_es)/5):
                    #pdb.set_trace()
                    return i 
                else:
                    q.append(xa_es[i])   
    if signal == 'ya':
        for i in range(len(ya_es)):
            if len(q) < 8:
                q.append(ya_es[i])
            else:
                avg,std = cavg(q)
                if (abs(ya_es[i] - avg) > 3*std and i > len(xl_es)/5):
                    #pdb.set_trace()
                    return i 
                else:
                    q.append(ya_es[i])   
    if signal == 'za':
        for i in range(len(za_es)):
            if len(q) < 8:
                q.append(za_es[i])
            else:
                avg,std = cavg(q)
                if (abs(za_es[i] - avg) > 3*std and i > len(za_es)/5):
                    #pdb.set_trace()
                    return i 
                else:
                    q.append(za_es[i])  
...
def main():
    ...
    parser.add_argument('--base', default='current_velocity_attack_sdc')
    ...
    for i in range(len(f)):
        ...
        xl_es,xa_es,yl_es,ya_es,zl_es,za_es = parse_es_twist(f[i][:-4]+'/current_velocity.txt')
        mid_frame_ = find_injection_raw(xl_es,xa_es,yl_es,ya_es,zl_es,za_es,f[i][:-4])
        amp = find_injection_range(f[i][:-4])
        #pdb.set_trace()
        if mid_frame_ == 0:
            mid_frame = int(len(xl)*18/51)
        else:
            mid_frame = int(mid_frame_/len(xl_es)*len(xl))
