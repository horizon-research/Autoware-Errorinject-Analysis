...
def find_injection_raw(filename):
    #pdb.set_trace()
    f = open(filename)
    a = f.readlines()
    s = 0
    ...
    for i in range(len(a)):
        if 'data:' in a[i]:
            s += 1
        if 'VehicleReady WaitOrder Stopping WaitDriveReady' in a[i]:
            b = i
    return b/s

def main():
    parser = argparse.ArgumentParser(description="base folder, save dir and database query")
    parser.add_argument('--base', default='decision_maker_state_attack_sdc')
    ...
    for i in range(len(f)):
        ...
        mid_frame_ = find_injection_raw(f[i][:-4]+'/decision_maker_state.txt')
        ...
        if mid_frame_ == 0:
            ...
            mid_frame = int(mid_frame_*len(xl))
