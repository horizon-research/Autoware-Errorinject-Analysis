...
def find_injection_range(filename):
    ...
    flag = 0
    ori_xp,ori_yp,ori_zp,ori_xo,ori_yo,ori_zo,ori_wo,ori_xl, ori_yl, ori_zl, ori_xa, ori_ya, ori_za = parse_finalwp('/localdisk/autoware_attack_sdc/baseline148/final_waypoints.txt')
    if signal == 'xl':
    ...
    if signal == 'xp':
        for i in range(len(xp_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(xp_vo[i] - avg) > 3*std and i > len(xp_vo)/5 and xp_vo[i] - xp_vo[i-1] > 0.5):
                    return i 
                else:
                    q.append(xp_vo[i])
    if signal == 'yp':
        for i in range(len(yp_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(yp_vo[i] - avg) > 3*std and i > len(xp_vo)/5 and yp_vo[i] - yp_vo[i-1] > 0.5):
                    return i 
                else:
                    q.append(yp_vo[i])
    if signal == 'zp':
        for i in range(len(zp_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(zp_vo[i] - avg) > 3*std and i > len(xp_vo)/5) and zp_vo[i] - zp_vo[i-1] > 0.5:
                    return i 
                else:
                    q.append(zp_vo[i])    
    if signal == 'xo':
        for i in range(len(xo_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(xo_vo[i] - avg) > 3*std and i > len(xp_vo)/5 and xo_vo[i] - xo_vo[i-1] > 0.5):
                    return i 
                else:
                    q.append(xo_vo[i])   
    if signal == 'yo':
        for i in range(len(yo_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(yo_vo[i] - avg) > 3*std and i > len(xp_vo)/5 and yo_vo[i] - yo_vo[i-1] > 0.5):
                    return i 
                else:
                    q.append(yo_vo[i])    
    if signal == 'zo':
        for i in range(len(zo_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(zo_vo[i] - avg) > 3*std and i > len(xp_vo)/5 and zo_vo[i] - zo_vo[i-1] > 0.5):
                    return i 
                else:
                    q.append(zo_vo[i]) 
    if signal == 'wo':
        for i in range(len(wo_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(wo_vo[i] - avg) > 3*std and i > len(xp_vo)/5 and wo_vo[i] - wo_vo[i-1] > 0.5):
                    #pdb.set_trace()
                    return i 
                else:
                    q.append(wo_vo[i])
    if signal == 'xl':
        for i in range(len(xl_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(xl_vo[i] - avg) > 3*std and i > len(xp_vo)/5 and xl_vo[i] - xl_vo[i-1] > 0.5):
                    return i 
                else:
                    q.append(xl_vo[i])      
    if signal == 'yl':
        for i in range(len(yl_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(yl_vo[i] - avg) > 3*std and i > len(xp_vo)/5 and yl_vo[i] - yl_vo[i-1] > 0.5):
                    return i 
                else:
                    q.append(yl_vo[i])    
    if signal == 'zl':
        for i in range(len(zl_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(zl_vo[i] - avg) > 3*std and i > len(xp_vo)/5 and zl_vo[i] - zl_vo[i-1] > 0.5):
                    return i 
                else:
                    q.append(zl_vo[i])    
    if signal == 'xa':
        for i in range(len(xa_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(xa_vo[i] - avg) > 3*std and i > len(xp_vo)/5 and xa_vo[i] - xa_vo[i-1] > 0.5):
                    return i 
                else:
                    q.append(xa_vo[i])   
    if signal == 'ya':
        for i in range(len(ya_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(ya_vo[i] - avg) > 3*std and i > len(xp_vo)/5 and ya_vo[i] - ya_vo[i-1] > 0.5):
                    return i 
                else:
                    q.append(ya_vo[i])   
    if signal == 'za':
        for i in range(len(za_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(za_vo[i] - avg) > 3*std and i > len(xp_vo)/5 and za_vo[i] - za_vo[i-1] > 0.5):
                    return i 
                else:
                    q.append(za_vo[i])  
    return 0                   
...
def main():
    ...
    parser.add_argument('--base', default='final_waypoints_attack_sdc')
    ...
    for i in range(len(f)):
        ...
        xp_wp,yp_wp,zp_wp,xo_wp,yo_wp,zo_wp,wo_wp,xl_wp,yl_wp,zl_wp,xa_wp,ya_wp,za_wp = parse_finalwp(f[i][:-4]+'/final_waypoints.txt')
        mid_frame_ = find_injection_raw(xp_wp,yp_wp,zp_wp,xo_wp,yo_wp,zo_wp,wo_wp,xl_wp,yl_wp,zl_wp,xa_wp,ya_wp,za_wp,f[i][:-4])
        amp = find_injection_range(f[i][:-4])
        #pdb.set_trace()
        if mid_frame_ == 0:
            mid_frame = int(len(xl)*18/51)
        else:
            mid_frame = int(mid_frame_/len(xp_wp)*len(xl))
