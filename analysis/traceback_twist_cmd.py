    ...
    if signal == 'za':
        for i in range(len(za_vo)):
            if len(q) < 8:
                ...
                avg,std = cavg(q)
                if (abs(za_vo[i] - avg) > 3*std) and i > len(za_vo)/5:
                    return i 
                else:
                    q.append(za_vo[i])  
    return 0                   
...
def find_injection_range(xl_vo, xa_vo, yl_vo, ya_vo, zl_vo, za_vo,filename):
    ...
    flag = 0
    ori_xl, ori_yl, ori_zl, ori_xa, ori_ya, ori_za = parse_twistraw('/localdisk/autoware_attack_sdc/baseline148/twist_cmd.txt')
    if signal == 'xl':
...
def main():
    ...
    parser.add_argument('--base', default='protect_twist_cmd')
    ...
    for i in range(len(f)):
        ...
        xl_tr,xa_tr,yl_tr,ya_tr,zl_tr,za_tr = parse_twistraw(f[i][:-4]+'/twist_cmd.txt')
        #pdb.set_trace()
